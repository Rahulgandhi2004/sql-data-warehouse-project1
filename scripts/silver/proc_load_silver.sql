------------------------------------------------------
--CLEANED THE BRONZE DATA AND INSERTED INTO THE SILVER
--Stored Procedure
------------------------------------------------------
...
BEGIN
	...
	sls_price)
		...
		sls_quantity,
			...
			CASE WHEN sls_price<0 OR sls_price<=0 THEN sls_sales / NULLIF(sls_quantity,0)
				else sls_price
			END AS sls_price
	...
	PRINT '>> Truncating table:silver.erp_px_cat_g1v2'
	TRUNCATE TABLE silver.erp_px_cat_g1v2;
	PRINT '>> Inserting inti the table:silver.erp_px_cat_g1v2'
...
PRINT 'Loading BATCH Duration: ' + CAST(DATEDIFF(second,@batchstart_time,@batchend_time)AS NVARCHAR)+ ' Seconds';
END TRY
